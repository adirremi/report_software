...
def zipcode_mistake(report1, report2):
    mistake_list = []
    ...
    for job1 in report1:
        ...
        for job2 in report2:
            # Check if the 'address,' 'total,' 'parts,' and 'zip code' are the same
            if (
                job1['address'] == job2['address']
                and are_integers_equal(job1['total'], job2['total'])
                ...
                #mistake_list.append([job1, "zipcode not the same"])
    ...
    return mistake_list, report1, report2#, print("zipcode check is done")